...
import { recordClientFormScoreSubmission, isScoreHistoryFormKey } from '../../../../utils/form-score-history'
...
export default defineEventHandler(async (event) => {
  ...
  if (formKey === 'ace') {
    const f = await prisma.aceForm.findFirst({ where: { userId: clientUserId }, orderBy: { id: 'desc' } })
    score = f?.totalScore ?? null
    severity = f?.severity ?? null
  } else if (formKey === 'gad') {
    const f = await prisma.gadForm.findFirst({ where: { userId: clientUserId }, orderBy: { id: 'desc' } })
    score = f?.totalScore ?? null
    severity = f?.severity ?? null
  } else if (formKey === 'phq') {
    const f = await prisma.phqForm.findFirst({ where: { userId: clientUserId }, orderBy: { id: 'desc' } })
    score = f?.totalScore ?? null
    severity = f?.severity ?? null
  } else if (formKey === 'pcl') {
    const f = await prisma.pclForm.findFirst({ where: { userId: clientUserId }, orderBy: { id: 'desc' } })
    score = f?.totalScore ?? null
    severity = f?.severity ?? null
  }
  ...
  if (formKey === 'ace') {
    const form = await prisma.aceForm.create({ data: { userId: clientUserId, status: 'IN_PROGRESS' } })
    await prisma.aceQuestion.create({ data: { formId: form.id, userId: clientUserId } })
    return { ok: true, formId: form.id }
  }
  if (formKey === 'gad') {
    const form = await prisma.gadForm.create({ data: { userId: clientUserId, status: 'IN_PROGRESS' } })
    await prisma.gadQuestion.create({ data: { formId: form.id, userId: clientUserId } })
    return { ok: true, formId: form.id }
  }
  if (formKey === 'phq') {
    const form = await prisma.phqForm.create({ data: { userId: clientUserId, status: 'IN_PROGRESS' } })
    await prisma.phqQuestion.create({ data: { formId: form.id, userId: clientUserId } })
    return { ok: true, formId: form.id }
  }
  if (formKey === 'pcl') {
    const form = await prisma.pclForm.create({ data: { userId: clientUserId, status: 'IN_PROGRESS' } })
