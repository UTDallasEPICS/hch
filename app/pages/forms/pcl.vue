<script setup lang="ts">
  ...
  async function submitForm() {
    ...
    try {
      ...
      const description =
        error?.data?.statusMessage || error?.statusMessage || 'Unable to submit. Please try again.'
      toast.add({ title: 'Submit failed', description, color: 'error' })
    } finally {
  ...
  onMounted(async () => {
    try {
      ...
      loadError.value = error?.data?.statusMessage || error?.statusMessage || 'Unable to load form.'
    }
  })
</script>
  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950">
    <main class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div class="mb-8">
        ...
        <p v-else class="text-primary-600 dark:text-primary-400 mt-2 text-sm font-medium">
        ...
        >
          ...
          <div class="mt-4 flex justify-between">
            <label v-for="opt in options" :key="opt.value" class="flex flex-col items-center gap-1">
              <span class="text-center text-sm text-gray-700 dark:text-gray-300">{{
                opt.label
              }}</span>
              <input
                type="radio"
