<script setup lang="ts">
  ...
  // Ensure slug is valid - redirect if missing
  watch(
    () => slug.value,
    (newSlug) => {
  ...
  // Fetch form (client-only so page always loads; API 404 shown in-page)
  const {
    data: form,
    pending: formPending,
    error: formError,
  ...
  // Load existing responses (client-only)
  const { data: existingResponses } = await useFetch(
    () => {
      if (!slug.value) return null
      return `/api/forms/${slug.value}/responses`
    },
    {
      watch: [slug],
      default: () => ({}),
  ...
  // Check if form is already completed (client-only)
  const { data: formStatus } = await useFetch(
    () => {
      if (!slug.value) return null
      return `/api/forms/${slug.value}/status`
    },
    {
      watch: [slug],
  ...
  function setResponse(alias: string, value: string) {
    ...
    saveTimeout = setTimeout(async () => {
      try {
        await $fetch(`/api/forms/${slug.value}/responses`, {
          method: 'POST',
  ...
  // Watch for existing responses changes (when loaded)
  watch(
    existingResponses,
    (newResponses) => {
  ...
  async function submit() {
    ...
    // Show modal if form is incomplete
    ...
    try {
      ...
      // Ensure all question aliases are in responses
      ...
      console.log('Submitting with responses:', allResponses)

      const res = await $fetch('/api/forms/submit', {
        method: 'POST',
        body: {
          formSlug: form.value.slug,
      ...
      if (res && (res as { ok?: boolean }).ok) {
        await navigateTo(`/forms/${slug.value}-results`)
      } else {
