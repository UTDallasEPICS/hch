<script setup lang="ts">
  ...
  const localPreviousNotes = ref<NoteItem[]>([...props.previousNotes])
  ...
  <div class="overflow-y-auto bg-gray-50 dark:bg-gray-950">
    ...
    >
      ...
      <div class="flex min-h-screen">
        ...
        <div
          v-show="sidebarOpen"
          class="fixed top-0 left-0 z-50 flex h-full w-64 flex-col border-r border-gray-200 bg-white shadow-xl md:relative md:z-auto md:h-full md:shadow-none dark:border-gray-800 dark:bg-gray-900"
        >
          <!-- X button inside sidebar-->
          <div
              ...
              <template v-if="filteredNotes.length > 0">
                ...
                >
                  <div class="flex items-center justify-between gap-1">
                    ...
                    <!-- Colored Attendance Indicator -->
                    <div class="flex items-center gap-1 text-xs font-medium">
                      <span
                        :class="note.attended ? 'text-green-500' : 'text-red-500'"
                        class="text-base leading-none"
                      >
                        ●
                      </span>
                      <span :class="note.attended ? 'text-green-500' : 'text-red-500'">
                        {{ note.attended ? 'Present' : 'Absent' }}
                      </span>
                    </div>
          ...
          <!-- Forms Tab -->
          <div v-if="sidebarTab === 'forms'" class="flex flex-1 flex-col gap-2 overflow-y-auto p-3">
            <div
              v-for="form in forms"
              :key="form.label"
        ...
        <div
          class="flex flex-1 flex-col divide-y divide-gray-200 overflow-hidden border-l border-gray-200 md:flex-row md:divide-x md:divide-y-0 dark:divide-gray-800 dark:border-gray-800"
        >
          <div
          ...
          >
            ...
            <div v-if="selectedNoteData" class="flex min-w-0 flex-col p-5 md:flex-1">
              ...
              <div v-else class="flex min-h-0 flex-1 flex-col gap-2">
                ...
                >
                  <NotesToolbar v-model="previousNoteContent" class="h-full w-full" />
                </div>
          ...
          <div
            v-if="selectedForm"
            class="w-64 flex-shrink-0 overflow-y-auto border-l border-gray-200 px-6 py-4 dark:border-gray-800"
          >
            <div class="mb-3 flex items-center justify-between">
              <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ selectedForm }}</h2>
              <button
                type="button"
                @click="selectedForm = null; formDetails = null"
                class="text-lg font-bold text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"
              >
                ×
              </button>
            </div>
            ...
            <div v-else-if="formDetails" class="space-y-3">
              <div
                v-if="formDetails.score != null || formDetails.severity"
