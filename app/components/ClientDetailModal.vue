<script setup lang="ts">
  ...
  async function saveAbsences() {
    ...
    try {
      ...
      const res = await $fetch<{ missedSessions: number }>(`/api/clients/${props.clientId}/absences`, {
        method: 'PATCH',
      ...
      <div class="flex flex-wrap items-center gap-4 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
        ...
        <div class="flex items-center gap-2">
          ...
          <template v-if="absencesEditing">
            ...
            <UButton size="sm" color="primary" :loading="absencesSaving" @click="saveAbsences">Save</UButton>
          ...
          <template v-else>
            ...
            <UButton size="xs" variant="ghost" icon="i-heroicons-pencil-square" aria-label="Edit absences" @click="absencesEditing = true" />
          </template>
        </div>
      </div>
